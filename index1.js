...
// --- FIX #2:  SIGNATURE VERIFICATION (USING RAW BODY) ---
...
async function handleIncomingMessage(msg) {
  ...
  try {
    ...
    // Insert order - using actual table schema: id, customer_phone, status, created_at, code
    ...
    if (!orderData || !orderData.id) {
      log("order_insert_no_id_returned", {
        orderData,
      });
      return;
    }
    ...
    log("order_created_with_confirmation", {
      orderId: orderData.id,
      code: orderCode,
      itemCount: parsed.length,
      phone,
